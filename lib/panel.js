...
export default class Panel {
	...
	renderCommands() {
		...
		const createButton = (cmd, icon) => {
			const btn = createElement("button", ["btn", "btn-default"], [
				createElement("span", ["icon", "icon-" + icon]),
				cmd
			]);
			btn.type = "button";
			btn.dataset.cmd = cmd;
		...
		frag.appendChild(
			createElement("div", ["btn-group"], [
				createButton("run", "triangle-right"),
				createButton("restart", "sync")
			])
		);
		frag.appendChild(
			createElement("div", ["btn-group"], [
				createButton("continue", "triangle-right")
			])
		);
		frag.appendChild(
			createElement("div", ["btn-group"], [
				createButton("next", "arrow-right"),
				createButton("step", "arrow-down")
			])
		);
