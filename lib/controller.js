...
export default class Controller {
	...
	initPanel(state = {}) {
		...
		const pp = PANEL_POSITION[panelPosition];
		...
		el.style[pp.cssProp] = `${state[pp.cssProp]}px`;
		const interactOptions = { edges: { [pp.resize]: true } };
		interact(el).resizable(interactOptions).on("resizemove", (event) => {
	...
	serialize() {
		...
		const panel = Object.assign({}, this.lastState.panel);
		panel[pp.cssProp] = this.panel.element.getBoundingClientRect()[pp.cssProp];
