...
export default class Controller {
	constructor(state) {
		...
		this.subscriptions.add(
			atom.commands.add("atom-workspace", {
				...
				"go-debug:halt": this.command.bind(this, "halt")
			}),
			...
			atom.config.observe("go-debug.panelPosition", this.initPanel.bind(this))
		);
	}

	initPanel() {
		if (this.atomPanel) {
			this.lastState.panel = this.panel.serialize();
			this.panel.destroy();
			this.atomPanel.destroy();
		}

		// debugger panel used to display details like variables, stacktrace, step/next/... buttons, ...
		const panelPosition = atom.config.get("go-debug.panelPosition");

		const el = createElement("div", ["go-debug-panel"]);
		this.panel = new Panel(el, this.lastState.panel);
		this.panel.onDidSelectStacktrace(this.setSelectedStacktraceIndex.bind(this));
		this.panel.onDidSelectGoroutine(this.setSelectedGoroutine.bind(this));
		this.atomPanel = atom.workspace[`add${panelPosition}Panel`]({
			item: el,
			visible: editingGo(atom.workspace.getActivePaneItem())
		});

		this.setSelectedStacktraceIndex(this.selectedStacktraceIndex);
	}
	...
	serialize() {
		...
		const panel = this.panel.serialize();
	...
	setSelectedStacktraceIndex(index) {
		...
		const goroutines = this.delve ? this.delve.goroutines : [];
		const currentGoroutineID = this.delve ? this.delve.getGoroutineID() : 0;

		this.panel.setDebuggerReady(!!this.delve);
		this.panel.renderStacktrace(stacktrace, this.selectedStacktraceIndex);
		this.panel.renderVariables(variables);
		this.panel.renderGoroutines(goroutines, currentGoroutineID);
		this.updateDebugLine();
	}
	setSelectedGoroutine(id) {
		if (!this.delve) {
			return;
		}

		// TODO: replace with events?
		this.delve.setGoroutine(id).then(this._onDelveUpdate.bind(this)).catch(catcher);
	}
	...
	command(fn) {
		...
		this.delve[fn]().then(this._onDelveUpdate.bind(this)).catch(catcher);
	}
	...
	// TODO: actually not an event...
	_onDelveUpdate() {
		// find the stacktrace entry that points to the last "user" code
		let index;
		if (this.delve && this.delve.state) {
			const { pc } = this.delve.state.currentGoroutine.userCurrentLoc;
			index = this.delve.stacktrace.findIndex((stack) => stack.pc === pc);
			if (index === -1) {
				index = 0;
			}
		}

		this.setSelectedStacktraceIndex(index || 0);
	}
}
