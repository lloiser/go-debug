...
export class DelveSession {
  ...
  _command (name: string): Promise<CommandFinished> {
    return this._call('Command', { name }).then((o: { State: DelveDebuggerState }) => {
      return this._stateToDebuggerState(o.State)
    }).catch((err) => {
  ...
  getState (): Promise<CommandFinished> {
    return this._call('State', {}).then((o: { State: DelveDebuggerState }) => {
      return this._stateToDebuggerState(o.State)
    })
  }
  _stateToDebuggerState (state: DelveDebuggerState) {
  ...
  _prepareGoroutines (o: { Goroutines: DelveGoroutine[] }): Goroutine[] {
    return o.Goroutines.map((goroutine) => {
      const loc = [goroutine.userCurrentLoc, goroutine.currentLoc, goroutine.goStatementLoc].find(loc => {
        if (!loc.file || !loc.function || !loc.function.name) {
          return false
        }
        return true
      })
