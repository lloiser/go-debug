...
export class Breakpoints extends EtchComponent {
  render () {
    ...
    const items = breakpoints.map(({ file, line, state, message }) => {
      return <div key={file + '|' + line} dataset={{ file, line }} title={message || ''} onclick={this.handleBreakpointClick}>
  ...
  handleRemoveBreakpointClick (ev) {
    ...
    if (file) {
      const line = +elementPropInHierarcy(ev.target, 'dataset.line')
      this.props.dbg.removeBreakpoint(file, line)
      ev.preventDefault()
      ev.stopPropagation()
    }
  }
    ...
    const noti = atom.notifications.addWarning(
      `The file ${file} does not exist anymore.`,
      {
        ...
        buttons: [{
          ...
          onDidClick: () => {
            noti.dismiss()
            getBreakpoints(this.props.store, file).forEach((bp) => this.props.dbg.removeBreakpoint(file, bp.line))
          }
        }, {
          text: 'No',
