...
export default function getDelve (goget, goconfig) {
  ...
  function located (dlvPath) {
    if (dlvPath) {
      return getVersion(dlvPath)
        .then((version) => update(version))
        .then(() => dlvPath)
    }
    return install().then(locate.bind(this))
  }

  function getVersion (dlvPath) {
    return goconfig.executor.exec(dlvPath, ['version'], 'project').then((r) => {
      if (r.exitcode !== 0) {
        return Promise.reject(r.stderr || (r.error && r.error.toString()))
      }
  ...
  if (customDlvPath) {
    return located(customDlvPath)
  }
  return locate().then(located.bind(this))
}
