...
export default class DelveConnection {
  constructor (spawn, connect, newSession, addOutputMessage) {
    this._spawn = spawn
...
function updateEnv (config, variables) {
  // already assign the process env here so they can be used by the config.env values
  variables.env = process.env // TODO: use goconfig!

  // TODO: ensure a GOPATH is set! https://github.com/lloiser/go-debug/issues/59
...
function getDlvArgs (config, variables) {
  const { mode, program, showLog = true, buildFlags, init: dlvInit, args } = config
  const { host, port } = hostAndPort(config)
