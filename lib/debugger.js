...
export class Debugger {
  ...
  addBreakpoint (file: string, line: number): Promise<void> {
    ...
    this._store.dispatch(Actions.addOutputMessage(`Adding breakpoint @ ${fileAndLine}\n`))

    return session.addBreakpoint({ bp })
      .then(({ id }) => {
        this._store.dispatch(Actions.addOutputMessage(`Added breakpoint @ ${fileAndLine}\n`))
  ...
  removeBreakpoint (name: string): Promise<void> {
    ...
    const fileAndLine = location(bp)
    this._store.dispatch(Actions.addOutputMessage(`Removing breakpoint @ ${fileAndLine}\n`))

    this._store.dispatch(Actions.editBreakpoint({ ...bp, state: 'busy' }))

    return session.removeBreakpoint({ bp })
      .then(() => this._store.dispatch(Actions.addOutputMessage(`Removed breakpoint @ ${fileAndLine}\n`)))
      .then(done)
      .catch((err) => {
